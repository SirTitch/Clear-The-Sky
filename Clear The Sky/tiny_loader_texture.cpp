...
void TinyObjLoader::load_obj(GLuint program3, string inputfile, std::string txtfile, int texloc, std::string texname, bool debugPrint)
{
	prog = program3;
	...
	objmodelID = glGetUniformLocation(prog, "model");
	objviewID = glGetUniformLocation(prog, "view");
	objprojectionID = glGetUniformLocation(prog, "projection");

	try
	{
		glActiveTexture(GL_TEXTURE2+texloc);

		/* load an image file directly as a new OpenGL texture */
		texID = SOIL_load_OGL_texture(txtfile.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
		...
		int loc = glGetUniformLocation(prog, texname.c_str());
		if (loc >= 0) glUniform1i(loc, texloc);
	}
	catch (exception &e)
	{
...
void TinyObjLoader::drawObject(GLuint program3,glm::mat4 model, glm::mat4 projection, glm::mat4 viewlook)
{
	...
	glUseProgram(prog);
	glUniformMatrix4fv(objmodelID, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(objviewID, 1, GL_FALSE, &viewlook[0][0]);
	glUniformMatrix4fv(objprojectionID, 1, GL_FALSE, &projection[0][0]);
	...
	//int loc = glGetUniformLocation(prog, "tex1");
